...
def read_wave(path, norm):
    ...
    with closing(wave.open(path, 'rb')) as wf:
        ...
        if num_channels == 2:
            ...
            return np.reshape(mono_data,(-1,)), sample_rate
        else:
            ...
            return np.reshape(wave_data,(-1,)), sample_rate
...
def skip_num(sr):
    ...
    T = np.arccos(percent) / (2*np.pi*freq)
    t = 1 / sr
    skip_num = (t/2 + T) // t
...
def detect_clip(audio, sr):

    # ClipDaT Algorithm
    
    maximum = max(audio)
    minimum = min(audio)
    argmax = np.reshape(np.argwhere(audio==maximum), (-1,))
    argmin = np.reshape(np.argwhere(audio==minimum), (-1,))
    argidx = np.concatenate((argmax, argmin))
    argidx = np.sort(argidx)
    clip_idx = np.zeros(np.shape(argidx)[0])
    
    last_idx = 0
    for i, idx in enumerate(argidx):
        if idx >= last_idx:
            # idx = idx[0]
            # n = 1
            clip_idx[i] = 0
            ...
            while(1):
                idx += 1
                if audio[idx] >= maximum * 0.995 or audio[idx] <= minimum * 0.995:
                    thre = 0
                    clip_idx[i] += 1
                else:
                    thre += 1
                    clip_idx[i] += 1
                    if thre == skip_num(sr) + 1:
                        clip_idx[i] -= thre
                        last_idx = idx
                        break
                    
    clip_idx = np.where(clip_idx <= skip_num(sr), 0, clip_idx)
    dur = np.sum(clip_idx)
    
    if dur == 0:
        return False, 0
    else:
        return True, round(dur / sr, 4)


def write_json(json_path, audio_id, snr, approximation, audio_is_clip, duration):
    
    with open(json_path,"w") as f:
        json_dict = {'audio_id': audio_id,'snr': snr, 'snr_approximation': approximation,
                     'clip': audio_is_clip, 'clip_duration': duration}
        dump(json_dict,f)
...
def main(args):
    ...
    snr = round(snr, 2)
    audio_is_clip, duration = detect_clip(audio, sr)
    # write .json
    audio_id = args[0][:-4]
    json_path = args[1]
    write_json(json_path, audio_id, snr, peak_is_single, audio_is_clip, duration)
    print(audio_id, snr, peak_is_single, audio_is_clip, duration)
